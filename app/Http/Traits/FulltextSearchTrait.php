...
trait FulltextSearchTrait
{
    ...
    public function searchFulltext(Request $request): array
    {
        $startTime = microtime(true);
        $data = $request->all();
        $originalKeyword = $data['keyword'] ?? "";
        $keyword = $this->formatKeywordFulltext($originalKeyword);
        if (empty($keyword)) {
            $words = $this->getPaginatedWords();
        } else {
            /* @var Paginator $words */
            $words = Word::query()
                ->whereRaw("MATCH(message, tags) AGAINST(? IN NATURAL LANGUAGE MODE)", [$keyword])
     ...
     * @return string|array
     */
    private function formatKeywordFulltext($kw): string|array
    {
