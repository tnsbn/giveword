...
;(function ($, window, document) {
  ...
  $.extend(Component.prototype, {
    ...
    listenMoreTakeword: function (el) {
      $(document).ready(function () {
        $(el).find('.btn-more-takeword').on('click', function (event) {
          event.preventDefault();
          let btnMore = $(this);
          let nextPage = btnMore.data('nextPage') === undefined ? 2 : btnMore.data('nextPage');
          btnMore.data('nextPage', 1 + btnMore.data('nextPage'));
          const keyword = $(el).find('#keyword').val();
    ...
    showModaldelete: function (id) {
      let messageTo = $(this.element).find('.word.' + id).find('.to .text').text();
